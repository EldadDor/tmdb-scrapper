...
export default function deleteDuplicate() {
	TV.aggregate([
		...
		item.forEach(async (item) => {
			item.id.shift();
			logger.warning(`Duplicate data found for ID ${item._id.id}`)
			await TV.deleteMany({_id: {$in: item.id}})
				.then(() => logger.success(`Duplicate data for ID ${item._id.id} deleted`))
				.catch((err) => logger.error(`Error deleting duplicate ID ${item._id.id}: ${err}`))
		})
	})
}